...
| **Plan Warp** | 計画が作業分解、順序、リスク、検証、撤回、完了証拠を持つかを見る。 | `audit-plan` |
| **Change Weft** | 差分と完了主張が、意味、契約、失敗処理、運用観測、試験義務、証拠を壊していないかを見る。 | `audit-diff` / `finish-check` |
...
uv run --python 3.13 --project . semantic-guard audit-decision-state --text "未決定: 採用基準。owner: 人間。next_action: 判断条件を書く。"
...
1. この `README.ja.md`
2. `docs/ja/naming.md`
3. `docs/ja/company-evidence.md`
4. `docs/ja/quickstart.md`
...
GitHub repository として見せる場合は、次の文書も確認する。

- `CONTRIBUTING.md`: 変更提案、試験、fixture / corpus の扱い。
- `SECURITY.md`: 安全連絡の扱い。この道具が security scanner ではないことも明記している。
- `CHANGELOG.md`: 公開 snapshot と今後の変更履歴。
- `docs/release/github-publication-checklist.md`: 公開前の確認表。

## 現在の位置付け

この snapshot は、公開用に整えた作業成果である。開発用 directory から、実行 package、試験、fixture、schema、公開向け文書、CI、導入時診断、companion skill を切り出している。

ただし、これは「一般文書に対する高精度監査器」の完成版ではない。現時点で主張できるのは、local calibration fixture と単体試験に通る、意味先行監査の実働試作である、という範囲までである。
