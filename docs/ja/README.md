...
この directory は、Criterion Loom を日本語で説明し、実演 command へ案内するための入口である。

技術名としての package / CLI / MCP server は `semantic-guard` のまま扱う。

## 対象読者と用途

対象読者は、Criterion Loom を初めて見る技術者と、公開 repository の日本語概要を掴みたい人である。この文書は、詳細設計を読む前に「どの日本語文書を読めばよいか」を判断するために使う。
...
この資料群は取説である。過大な断定を避け、実行できること、検証済みの範囲、未検証の範囲を分けて読むために使う。
