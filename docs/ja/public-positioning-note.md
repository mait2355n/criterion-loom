...
Codex skillから監査し、結果をJSONの`findings`、`missing`、`next_actions`、
`details`として返す意味先行監査ツールである。

## 公開名と四本柱

- **Criterion Loom**: README、docs、release説明でこの道具群を指す公開名。
...
- `llm-explore-request`は、入力と文脈から読み取れるfact / inference / hypothesis / unknown / pending decisionの候補をLLMが分類し、schema-validな探索JSONとして質問、限界、非決定事項を返す。網羅性や正しさの証明ではない。
...
- LLM reviewerの出力は補助材料として記録し、採用、差戻し、保留、最終受入の判断は人間側へ残す。
...
CLI使用法の誤りはstderrのargparse messageとexit code 2で返る。監査対象の警告やblockerはstdoutのJSON `status`、`findings`、`missing`に残る。MCP/CLIの詳しい契約は`skills/semantic-implementation/references/mcp-contract.md`に置き、このメモでは再定義しない。
