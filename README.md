...
- whether an implementation plan covers risk, validation owner, progress control, change control, rollback, and evidence
- whether a diff may break meaning, quality, security, tests, or documentation
...
| **Plan Warp** | planning audit for work breakdown, sequence, validation, risk, progress, rollback, and evidence | `audit-plan` |
| **Change Weft** | implementation audit for diff meaning, public contracts, failure handling, tests, and completion evidence | `audit-diff` and `finish-check` |
...
For Japanese usage, demonstration, and company-facing evidence notes, see `README.ja.md` and `docs/ja/`.
...
| **Plan Warp** | `audit-plan` | Does the plan cover deliverables, work breakdown, risk, verification, validation, rollback, and evidence? |
| **Change Weft** | `audit-diff` | What could this change break in meaning, quality, security, tests, docs, or operations? |
...
The GitHub CI workflow runs compile, unit tests, fixture evaluation, and `doctor` on Python 3.11 and 3.13.
...
The audit model is influenced by requirements engineering, project planning, software life-cycle processes, software construction, product quality models, code review practice, software operations, and secure development guidance. The tool does not implement those standards wholesale. It compresses a few useful checks into a local Codex-facing guard surface.
