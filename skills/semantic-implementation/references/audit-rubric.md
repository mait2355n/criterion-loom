...
The original product purpose covers requirements, plans, actions, realization methods, changes, verification, completion, and bounded action assurance. Canonical v1 requirement-relation auditing exposes only the requirement-relation vertical slice; the independent direction-binding audit is a separate additive v1 contract. Treat plan, diff, finish, operation, and other lifecycle assessments as manual work or as explicitly selected archived-runtime compatibility work until a corresponding v1 workflow is implemented and adopted.
